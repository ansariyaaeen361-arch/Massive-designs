...
export default function ServiceCta({ heading, paragraphs, buttonLabel, buttonHref, onButtonClick }) {
  const handleClick = () => {
    trackEvent(buttonHref?.startsWith('tel:') ? 'call_click' : buttonHref?.startsWith('mailto:') ? 'email_click' : 'cta_click', {
      source: 'service_cta',
      label: buttonLabel,
    });
    onButtonClick?.();
